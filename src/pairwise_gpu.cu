...
template<int M, int F>
PottsPotentialGPU<M, F>::PottsPotentialGPU(const float* features, int N, float w) : PairwisePotentialBase(N), w_(w) {
...
template<int M, int F>
template<class T>
PottsPotentialGPU<M, F>* PottsPotentialGPU<M, F>::FromImage(int w, int h, float weight, float posdev, const T* features, float featuredev) {
...
PottsPotentialGPU<M, F>* PottsPotentialGPU<M, F>::FromUnorganizedData(int N, float weight, const PT* positions, float posdev, int posdim,
